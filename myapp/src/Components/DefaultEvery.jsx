...
const DefaultEvery = ({ toggler }) => {
  ...
  const getData = async (page = 1) => {
    try {
      let res = await fetch(`https://newsapi.org/v2/everything?q=bitcoin&apiKey=${process.env.REACT_APP_apiKey}&pageSize=20&page=${page}`);
      res = await res.json();
      console.log(res)
      setLoading(false);
      if (res.status == 'error') {
